...
def _build_slots() -> tuple[SlotDefinition, ...]:
    ...
    return (
        ...
        _slot("problem_statement", "request", "What must be predicted.", "string", R, "What would you like the system to predict?", priority_weight=95),
        ...
        _slot("target_column", "target", "Target field name or identifier.", "string", R, "Which column contains the value you want to predict?", priority_weight=100),
        ...
        _slot("time_column", "time", "Timestamp or period field.", "string", R, "Which column tells us when each observation happened?", priority_weight=100),
        _slot("frequency", "time", "Native observation frequency.", "duration", R, "How often is a new observation recorded?", priority_weight=100),
        ...
        _slot("forecast_horizon", "time", "Number of future periods to predict.", "duration", R, "How far into the future should we predict?", priority_weight=100),
        ...
        _slot("dataset_type", "series", "Whether the data covers one item, several items, or groups of items.", "enum", R, "Does your data cover one item, several separate items, or groups with levels?", allowed_values=("single_series", "panel", "hierarchical"), priority_weight=85),
        _slot("series_id_columns", "series", "Columns that identify each item or series.", "string_list", C, "Which column identifies each item?", activation_rule="panel_or_hierarchical"),
        _slot("hierarchy_columns", "series", "Columns showing group levels from broadest to most specific.", "string_list", C, "Which columns show the group levels from broadest to most specific?", activation_rule="hierarchical"),
        _slot("aggregation_level", "series", "Level at which forecasts are required.", "string", C, "At which group level should results be returned?", activation_rule="granularity_changes_or_hierarchy"),
        ...
        _slot("source_mode", "source", "How the data will be provided.", "enum", R, "How will the data be provided: upload a file, connect to an online service, use a database, or choose from our data list?", allowed_values=("upload", "api", "database", "catalog"), priority_weight=120),
        _slot("source_reference", "source", "Non-secret source identifier.", "string", C, "Which file, online service, or database connection should be used?", activation_rule="source_reference_required", priority_weight=115),
        ...
        _slot("forecast_type", "output", "Whether to return one prediction, an uncertainty range, or both.", "enum", R, "Do you want one predicted value, a range showing uncertainty, or both?", allowed_values=("point", "probabilistic", "both"), priority_weight=70),
        ...
        _slot("output_granularity", "output", "How results are grouped by time and item.", "string", R, "How should results be grouped by time and item?", priority_weight=60),
        _slot("primary_metric", "evaluation", "How prediction quality should be compared.", "enum", R, "How should we judge which prediction is better?", allowed_values=("mae", "rmse", "mase", "smape"), priority_weight=55),
