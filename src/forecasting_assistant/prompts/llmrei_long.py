...
_ANSWER_EXAMPLES = {
    ...
    "lead_time": "1 hour",
    "dataset_type": "several separate items",
    "series_id_columns": "asset_id",
    "hierarchy_columns": "country, store, product",
    "aggregation_level": "each product",
    ...
    "forecast_type": "one predicted value",
    ...
    "output_granularity": "one result per day for each item",
    "primary_metric": "smallest average prediction error",
...
def example_answer(request: QuestionRequest) -> str:
    ...
    if request.available_dataset_columns and request.slot_id in {
        "target_column",
        "time_column",
        "series_id_columns",
        "hierarchy_columns",
    }:
        if request.slot_id in {"series_id_columns", "hierarchy_columns"}:
            return ", ".join(request.available_dataset_columns[:3])
        return request.available_dataset_columns[0]
...
def build_question_instructions() -> str:
    return (
        ...
        "Use everyday language for a novice user. Do not use unexplained terms such as slot, schema, hierarchy, panel, point forecast, probabilistic forecast, covariate, or metric. Explain the idea in simple words or add a short parenthetical explanation.\n"
...
def validate_question(output: QuestionOutput, request: QuestionRequest) -> bool:
    ...
    mentioned_choices = {
        ...
        if _contains_phrase(question_part, candidate)
    }
    if len(mentioned_choices) == 1:
        candidate = next(iter(mentioned_choices))
        if candidate.lower() not in confirmed_text:
            return False
    return True
